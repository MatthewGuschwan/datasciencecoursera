...
```
After imputing data, the mean is `r Imean` and the media is `r Imedian`.  The mean and median are now the same, which seems strange until you consider that the numbers I imputed will add up to that average.  On the days in which there was no data recorded (NAs), that data was replaced by the average.
